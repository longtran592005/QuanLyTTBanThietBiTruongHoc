...
{
    public class CategoryManagementPage : UserControl
    {
        ...
        {
            var split = new SplitContainer { Dock = DockStyle.Fill, SplitterDistance = 620, BackColor = UITheme.BackgroundColor };
            ...
            _emptyState.SetContent("Chưa có danh mục", "Tạo danh mục mới để bắt đầu.");
            ...
            var title = new Label { Dock = DockStyle.Top, Height = 28, Text = "Chi tiết danh mục", Font = UITheme.SectionTitleFont, ForeColor = UITheme.TextPrimaryColor };
        ...
        private void LoadData()
        {
            ...
            _grid.DataSource = _items.Select(x => new { x.CategoryId, x.CategoryName, x.Description }).ToList();
            if (_grid.Columns.Count >= 3)
            {
                _grid.Columns["CategoryId"].HeaderText = "Mã DM";
                _grid.Columns["CategoryName"].HeaderText = "Tên danh mục";
                _grid.Columns["Description"].HeaderText = "Mô tả";
            }
        ...
        private Category GetSelected()
        {
            ...
            var id = Convert.ToInt32(_grid.SelectedRows[0].Cells["CategoryId"].Value);
        ...
        private void UpdateDetails()
        {
            ...
            _details.Text = selected == null ? "Chọn một danh mục để xem chi tiết." : $"Tên: {selected.CategoryName}\nMô tả: {selected.Description}";
        }

        private void AddItem()
        {
            using (var editor = new CategoryEditorForm())
            {
        ...
        private void EditItem()
        {
            ...
            using (var editor = new CategoryEditorForm(selected))
            {
                if (editor.ShowDialog() == DialogResult.OK)
                {
                    editor.Category.CategoryId = selected.CategoryId;
        ...
        private void DeleteItem()
        {
            ...
            if (!UiDialogs.Confirm($"Bạn có chắc chắn muốn xóa danh mục \"{selected.CategoryName}\"?\n\nThao tác này không thể hoàn tác.", "Xóa danh mục"))
                return;
            ...
            try
            {
                ...
                LoadData();
                UiDialogs.ShowSuccess("Đã xóa danh mục thành công.", "Xóa thành công");
            }
            catch (Exception ex)
            {
                AppLogger.Error($"Failed to delete category {selected.CategoryId}", ex);
                UiDialogs.ShowError($"Không thể xóa danh mục.\n\n{ex.Message}", "Xóa thất bại");
            }
        }
    }
}
